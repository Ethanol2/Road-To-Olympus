...
/// <summary>
/// Custom Inspector for Transform. Reset or Snap the transform values.
/// </summary>
...
public class TransformReset : DecoratorEditor
{
	...
	private float RotValue(float v, int decimalPoints)
	{
		v = 0.5f * (float) System.Math.Round(v / 0.5f);
		return v;
	}
	...
	public override void OnInspectorGUI()
	{
		...
		if (GUILayout.Button("Reset Position", EditorStyles.miniButtonLeft))
		{
			p.vector3Value = resetPosition;
			serializedObject.ApplyModifiedProperties();
			GUI.FocusControl(null);
		}
		if (GUILayout.Button("Reset Rotation", EditorStyles.miniButtonMid))
		{
			r.quaternionValue = Quaternion.Euler(resetRotation);
			serializedObject.ApplyModifiedProperties();
			GUI.FocusControl(null);
		}
		if (GUILayout.Button("Reset Scale", EditorStyles.miniButtonRight))
		{
		...
		EditorGUILayout.BeginHorizontal();
		if (GUILayout.Button("Snap Position", EditorStyles.miniButtonLeft))
		{
			p.vector3Value = new Vector3((float) System.Math.Round(p.vector3Value.x, 1), (float) System.Math.Round(p.vector3Value.y, 1), (float) System.Math.Round(p.vector3Value.z, 1));
			serializedObject.ApplyModifiedProperties();
			GUI.FocusControl(null);
		}
		if (GUILayout.Button("Snap Rotation", EditorStyles.miniButtonMid))
		{
			Vector3 rV3 = r.quaternionValue.eulerAngles;
			r.quaternionValue = Quaternion.Euler(RotValue(rV3.x, 1), RotValue(rV3.y, 1), RotValue(rV3.z, 1));
			serializedObject.ApplyModifiedProperties();
			GUI.FocusControl(null);
		}
		if (GUILayout.Button("Snap Scale", EditorStyles.miniButtonRight))
		{
			s.vector3Value = new Vector3((float) System.Math.Round(s.vector3Value.x, 1), (float) System.Math.Round(s.vector3Value.y, 1), (float) System.Math.Round(s.vector3Value.z, 1));
			serializedObject.ApplyModifiedProperties();
			GUI.FocusControl(null);
		}
		EditorGUILayout.EndHorizontal();
